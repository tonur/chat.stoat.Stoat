...
and lock them out of Stoat making the program unusable.

This will dramatically improve the experience :

 1) The maintainer doesn't need to be worried at all times of an update which will break Stoat.
...
settings_path = Path(f"{XDG_CONFIG_HOME}/stoat/settings.json")
settings_path_temp = Path(f"{XDG_CONFIG_HOME}/stoat/settings.json.tmp")
try:
